import numpy as np
from numba import jit, float32, boolean, int32, njit
from numba.typed import List
from now.hko.evaluation import rainfall_to_pixel
from now.config import cfg
import torch

thresholds_global = cfg.HKO.EVALUATION.THRESHOLDS
thresholds_global = [rainfall_to_pixel(threshold) for threshold in thresholds_global]
thresholds_nb_global = List()
[thresholds_nb_global.append(x) for x in sorted(thresholds_global)]
base_balancing_weights_global = cfg.HKO.EVALUATION.BALANCING_WEIGHTS
base_balancing_weights_nb_global = List()
[base_balancing_weights_nb_global.append(x) for x in base_balancing_weights_global]
...
def get_GDL_numba(prediction, truth, mask):
    ...
    return _get_GDL_numba(prediction, truth, mask, seqlen, batch_size, height, width)


@njit()
# @jit(float32(float32, float32, boolean))
def _get_GDL_numba(prediction, truth, mask, seqlen, batch_size, height, width):
...
def get_hit_miss_counts_numba(prediction, truth, mask, thresholds=None):
    ...
    if thresholds is None:
        thresholds_nb = thresholds_nb_global
    else:
        thresholds = [rainfall_to_pixel(threshold) for threshold in thresholds]
        thresholds_nb = List()
        [thresholds_nb.append(x) for x in sorted(thresholds)]
    ...
    ret = _get_hit_miss_counts_numba(prediction=prediction,
                                     truth=truth,
                                     mask=mask,
                                     thresholds=thresholds_nb, seqlen=seqlen, batch_size=batch_size,
                                     height=height, width=width, threshold_num=len(thresholds_nb))
...
def _get_hit_miss_counts_numba(prediction, truth, mask, thresholds,
                                 seqlen, batch_size, height, width, threshold_num):
...
def get_balancing_weights_numba(data, mask, base_balancing_weights=None, thresholds=None):
    ...
    if base_balancing_weights is None:
        base_balancing_weights_nb = base_balancing_weights_nb_global
    else:
        base_balancing_weights_nb = List()
        [base_balancing_weights_nb.append(x) for x in sorted(base_balancing_weights)]
    ...
    ret = _get_balancing_weights_numba(data=data,
                                       mask=mask,
                                       base_balancing_weights=base_balancing_weights_nb,
                                       thresholds=thresholds_nb, seqlen=seqlen, batch_size=batch_size,
                                       height=height, width=width, threshold_num=len(thresholds_nb))
    return ret
...
@njit()
# @jit(float32(float32, boolean, float32, float32, int32, int32, int32, int32, int32))
