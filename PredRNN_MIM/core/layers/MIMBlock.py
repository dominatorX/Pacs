...
from core.layers.CABC import CABC
...
class MIMBlockBase(nn.Module):
    def __init__(self, in_channel, num_hidden, width, filter_size, stride, device, tln=False):
        super().__init__()
        if tln:
            if tln == 1:
                norm = nn.LayerNorm
...
class MIMBlock(MIMBlockBase):
    def __init__(self, in_channel, num_hidden, width, filter_size, stride, device, tln=False):
        super(MIMBlock, self).__init__(in_channel, num_hidden, width, filter_size, stride, device, tln)
